<?php 
  ...
  if(isset($_POST['btnSave'])!=""){
    ...
    $UpdateSQL = "UPDATE skills SET SkillName='$SkillName'
                  WHERE SkillName=$kid";
    $result = mysqli_query($connection, $UpdateSQL);   
      
    if ($result) {   
      header("location: SkillsAdmin.php");
    } else {
      $error="Error: ". mysqli_error($connection)." Please report to admin@gradforce.com. Thank you!";
    }
  }

  $SelectSql = "SELECT SkillName FROM skills WHERE SkillName=$kid";
...
<body>
  ...
  <div class="container">
        <br>
    <div class="row">
      <h1 id="IdSkillAdmin" align="center" style="color:#FFF"><strong>Update Skill</strong></h1>
      <div class="col-sm-3">
      </div>
      <div class="panel panel-primary col-sm-6">
        ...
        <div class="panel-body" style="color:black">
          <form id="SkillForm" action="" method="post" enctype="multipart/form-data" data-toggle="validator" autocomplete="off" >
            <div class="form-group col-sm-12">
              <label class="label label-primary">Skill</label>
              <input type="text" class="form-control" id="IdFName" name="SkillName" value="<?php echo $row['SkillName']; ?>" required autofocus>
            </div>

            <hr class="style1">

            <div class="form-group col-sm-12" align="right">
              <button type="button" name="btnCancel" class="btn btn-primary" onclick="goBack();">Cancel</button>
              <button type="submit" name="btnSave" class="btn btn-primary" onclick="" >Save</button>   
            </div>     
          </form>
        </div>
      </div>
